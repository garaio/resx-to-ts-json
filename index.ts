...
export function executeResxToTsTranslationKeys(typeScriptResourcesNamespace: string, virtualResxFolder: string, virtualTypeScriptFolder: string, fileNameRegex: any = /.resx$/): void {
    let files = getFilesFromFolder(virtualResxFolder, fileNameRegex);
...
export function executeResxToTs(typeScriptResourcesNamespace: string, virtualResxFolder: string, virtualTypeScriptFolder: string, fileNameRegex: any = /.resx$/): void {
    let files = getFilesFromFolder(virtualResxFolder, fileNameRegex);
...
export function executeResxToJson(virtualResxFolder: string, virtualJsonFolder: string, fileNameLanguage?: string, fileNameRegex: any = /.resx$/): void {
    let files = getFilesFromFolder(virtualResxFolder, fileNameRegex);
...
function getFilesFromFolder(virtualResxFolder: string, fileNameRegex: any): any {
    let files: any = null;
...
function convertResxToTypeScriptModelTranslationKeys(resxFilename: string, typeScriptResourcesNamespace: string, virtualTypeScriptFolder: string): void {
    fs.readFile(resxFilename, function (err: any, data: any) {
        const parser = new xml2js.Parser();
...
function convertDictionaryToTsMappingTranslationKeys(typeScriptResourcesNamespace: string, dictionary: Dictionary, nest: number) {
    let nestedTabs = "";
    for (var i = 0; i < nest; i++) {
        nestedTabs += "\t";
    }
...
function convertXmlToTypeScriptModelFileTranslationKeys(xmlObject: any, resxFilename: string, typeScriptResourcesNamespace: string, virtualTypeScriptFolder: string): void {
    const projectRoot = getProjectRoot();
    const relativeResxFilename = resxFilename.replace(projectRoot, "").replace(/\\/g, "/");
    const className = resxFilename.substr(resxFilename.lastIndexOf("\\") + 1).replace('.resx', '').replace(".", "_");
...
function getProjectRoot(): string {
    ...
    for (let i = 0, length = splittedDirName.length - 1; i < length; i++) {
